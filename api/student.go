...
func AddStudent(ctx context.Context) {
	...
	if err != nil {
		log.Printf("[AddStudent] add stu(%v) error(%v)", goutil.Struct2Json(stu), err)
...
func UpdateStudent(ctx context.Context) {
	...
	if err != nil {
		log.Printf("[UpdateStudent] update stu(%v) error(%v)", goutil.Struct2Json(stu), err)
		WriteResultWithSrvErr(ctx, err)
		return
	}
...
func DeleteStudent(ctx context.Context) {
	...
	if err != nil {
		log.Printf("[DeleteStudent] delete ids(%v) error(%v)", ids, err)
		WriteResultWithSrvErr(ctx, err)
		return
	}
...
func GetStudent(ctx context.Context) {
	...
	if err != nil {
		log.Printf("[GetStudent] get stu(%v) error(%v)", id, err)
...
func GetStudents(ctx context.Context) {
	...
	if err != nil {
		log.Printf("[GetStudents] get stu by(%v) error(%v)", argMap, err)
		WriteResultWithSrvErr(ctx, err)
		return
	}
	WriteResultSuccess(ctx, goutil.Map{
